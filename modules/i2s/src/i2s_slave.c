    ...
    xclock_t bclk)
{
    ...
    size_t i;
    size_t idx;
    size_t const num_data_bits = I2S_DATA_WIDTH;
    ...
    // In lib_i2s, we wholly split the two loops - I think to get timing optimal
    // Reproduce the same here, and investigate timing optimisations in future
    if (num_data_bits == 32)
    {
        for (;;)
        {
    ...
    else // num_data_bits != 32
    {
        ...
        for (;;)
        {
            ...
            {
                port_set_trigger_time(p_dout[i], initial_out_port_time);
                port_out_part_word(p_dout[i], bitrev(out_samps[idx] << data_bit_offset), num_data_bits);
            }

            port_set_trigger_time(p_lrclk, initial_in_port_time);
            port_set_shift_count(p_lrclk, num_data_bits);
