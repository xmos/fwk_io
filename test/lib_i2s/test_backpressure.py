...
rx_tx_inc_args = {
    "rx_delay_inc_50ns,tx_delay_inc_50ns": (5, 5),
...
@pytest.mark.parametrize(
    "num_channels", num_channels_args.values(), ids=num_channels_args.keys()
)
@pytest.mark.parametrize(
    ("receive_increment", "send_increment"),
    rx_tx_inc_args.values(),
    ids=rx_tx_inc_args.keys(),
)
def test_i2s_backpressure(
    build,
    nightly,
    capfd,
    request,
    ...
    subprocess.run(
        (
            ...
            "--plugin",
            "LoopbackPort.dll",
            "-port tile[0] XS1_PORT_1G 1 0 -port tile[0] XS1_PORT_1A 1 0",
        )
    )
