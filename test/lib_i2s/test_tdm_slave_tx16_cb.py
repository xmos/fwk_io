...
tx_offset_args = {"0": (0), "1": (1), "2": (2)}


@pytest.mark.parametrize(
    ("tx_offset"), tx_offset_args.values(), ids=tx_offset_args.keys()
)
    ...
    if DEBUG:
        ...
        with capfd.disabled():
            Pyxsim.run_with_pyxsim(
                ...
                simargs=[
                    "--vcd-tracing",
                    f"-o tdm_trace_{tx_offset}.vcd -tile tile[0] -cycles -ports -ports-detailed -cores -instructions",
                ],
                timeout=100,
            )
        ...
        Pyxsim.run_with_pyxsim(binary, simthreads=[checker], simargs=[], timeout=100)
