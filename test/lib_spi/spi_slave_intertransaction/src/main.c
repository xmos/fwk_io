...
port_t master_requested_exit_port = XS1_PORT_1G;
...
DECLARE_JOB(exit_req_handler, (void));

/* Note: This must intentionally poll to burn cycles */
void exit_req_handler(void) {
    port_enable(master_requested_exit_port);
    while(port_peek(master_requested_exit_port) == 0) {;}
    _Exit(1);
    for(;;);
}
...
static void alert_tester_of_error(void) {
    port_out(setup_strobe_port, 1);
    while(1) {;}
}
